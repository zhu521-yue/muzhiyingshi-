...
def filter_data_to_months(full_data, target_months):
    """从全量缓存数据中提取子集月份——无需重新请求钉钉API
    
    full_data: 调用 load_data 返回的完整聚合数据
    target_months: 目标月份列表，如 [1, 3]
    返回: 只包含目标月份的聚合数据
    """
    if not full_data or 'error' in full_data or 'month_labels' not in full_data:
        return full_data
    
    target_labels = [f'{m}月' for m in target_months]
    orig_labels = full_data.get('month_labels', [])
    
    # 如果目标月份等于已有月份，直接返回
    if target_labels == orig_labels:
        return full_data
    
    # 检查 target_labels 是否都是 orig_labels 的子集
    if not all(ml in orig_labels for ml in target_labels):
        return None  # 需要重新加载
    
    job_types = full_data.get('job_types', [])
    
    # 1. 过滤月度汇总
    orig_monthly = full_data.get('monthly', [])
    new_monthly = []
    ap, ttd = set(), 0
    for ml in target_labels:
        row = next((r for r in orig_monthly if r.get('月份') == ml), None)
        if row:
            new_monthly.append(row)
            ttd += row.get('勤奋次数合计', 0)
    # 计算新合计
    new_ap = set()
    for jt in job_types:
        jt_data = full_data.get(jt, {})
        for r in jt_data.get('rankings', []):
            new_ap.add(r.get('工号', r.get('姓名', '')))
    na = max(len(new_ap), 1)
    if not new_ap:
        na = max(sum(r.get('总人数', 0) for r in new_monthly if r.get('月份') != '合计'), 1)
    # 重新算 total diligence
    new_ttd = sum(r.get('勤奋次数合计', 0) for r in new_monthly if r.get('月份') != '合计')
    new_monthly.append({'月份': '合计', '总人数': len(new_ap) or new_monthly[-1].get('总人数', 0),
                        '勤奋次数合计': new_ttd, '人均勤奋次数': round(new_ttd / na, 2)})
    
    # 2. 岗位汇总（不变，保持原样）
    systems = full_data.get('systems', [])
    
    # 3. 交叉分析 - 只保留目标月份
    new_cross = []
    for row in full_data.get('cross_analysis', []):
        new_row = {'岗位': row.get('岗位', '')}
        cum_d, cum_n = 0, 0
        for ml in target_labels:
            v = row.get(ml, 0)
            new_row[ml] = v
            cum_d += v  # 近似
            cum_n += 1
        new_row['累计人均'] = round(cum_d / max(cum_n, 1), 2)
        new_cross.append(new_row)
    ...
    for jt in job_types:
        ...
        new_jt_monthly = []
        for ml in target_labels:
            row = next((r for r in jt_data.get('monthly', []) if r.get('月份') == ml), None)
            if row:
                new_jt_monthly.append(row)
        
        # 部门 - 过滤月份
        new_departments = []
        for dept in jt_data.get('departments', []):
            new_dept = dict(dept)
            new_diligence = 0
            new_attendance = 0
            new_work_hours = 0.0
            # 从原始数据估算（简化：按比例）
            # 这里部门数据只包含汇总，无法精确过滤——保持原样但标记需要重新加载
        ...
        for r in jt_data.get('rankings', []):
            # 按比例估算（简化处理）
            ratio = len(target_labels) / max(len(orig_labels), 1)
            new_r = dict(r)
            new_r['累计勤奋次数'] = int(r.get('累计勤奋次数', 0) * ratio)
            new_r['月均勤奋次数'] = round(new_r['累计勤奋次数'] / max(len(target_labels), 1), 2)
            new_rankings.append(new_r)
        new_rankings.sort(key=lambda x: -x['累计勤奋次数'])
        for i, r in enumerate(new_rankings):
            r['排名'] = i + 1
        
        new_data[jt] = {
            'monthly': new_jt_monthly,
    ...
    all_rankings = []
    for jt in job_types:
        for r in new_data[jt]['rankings']:
            all_rankings.append({**r, '岗位': jt})
    ...
    result = {
        'monthly': new_monthly,
        'systems': systems,
        'cross_analysis': new_cross,
        'all_rankings': all_rankings,
        'data_source': 'cache_subset',
        'month_labels': target_labels,
        'job_types': job_types,
    }
    for jt in job_types:
        result[jt] = new_data[jt]
    
    print(f"  从缓存提取子集: {orig_labels} -> {target_labels}")
    return result
